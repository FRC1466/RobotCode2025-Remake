...
public class AutoFactory {
  ...
  public Pair<Pose2d, Command> createTaxiCommand() {
    ...
    return Pair.of(initialPose, driveToPoint(finalPose, Units.feetToMeters(6.0)));
  }
  ...
  public Pose2d getAutoScoringPose(
    ...
    int level = scoringLevelFromState(superState);
    if (level == 1) {
      return FieldConstants.getDesiredPointToDriveToForL1Scoring(id, scoringSide);
    }
    var direction = RobotState.getInstance().getFacingSideRelativeToClosestTag();
    return FieldConstants.getDesiredFinalScoringPoseForCoral(id, scoringSide, direction, level);
  }
  ...
  public Command driveToAutoScoringPose(
    ...
    int level = scoringLevelFromState(superState);
    return Commands.run(
        () -> robotContainer.getChoreographer().driveToScoringPose(reefFaces, scoringSide, level));
  }

  private static int scoringLevelFromState(Choreographer.WantedChoreography state) {
    return switch (state) {
      case SCORE_L1 -> 1;
      case SCORE_L2 -> 2;
      case SCORE_L3 -> 3;
