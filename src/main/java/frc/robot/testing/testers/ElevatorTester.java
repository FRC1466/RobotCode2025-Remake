...
/**
 * Tests the Elevator subsystem in isolation.
 * Validates motor function, position control, sensor readings, and limits.
 */
public class ElevatorTester extends SubsystemTester {
  ...
  protected void defineTestSteps() {
    ...
    testSteps.add(new TestStep("Sensor Connectivity Check", true) {
      @Override
      ...
      protected boolean validateStep() {
        ...
        if (Math.abs(position) > Units.inchesToMeters(100)) {
          setFailureReason("Position reading unreasonable: " + 
              Units.metersToInches(position) + " inches");
          return false;
        }
        
        System.out.println("✓ Sensors connected. Position: " + 
            String.format("%.2f", Units.metersToInches(position)) + " in, Velocity: " + 
            String.format("%.2f", Units.metersToInches(velocity)) + " in/s");
    ...
    testSteps.add(new TestStep("Motor Current Check", false) {
      @Override
      protected void onStart() {
    ...
    testSteps.add(new TestStep("Position Control Test (Mid)", false) {
      ...
      protected void onStart() {
        ...
        System.out.println("Moving to mid position: " + 
            String.format("%.1f", Units.metersToInches(targetPosition)) + " inches");
      }
      ...
      @Override
      protected boolean isStepComplete() {
        return elevator.atGoal(POSITION_TOLERANCE) || stepTimer.hasElapsed(TEST_TIMEOUT);
      }
      ...
      protected boolean validateStep() {
        ...
        if (timedOut && !atGoal) {
          setFailureReason("Failed to reach target position within timeout. Current: " + 
              String.format("%.2f", Units.metersToInches(elevator.getPosition())) + 
              " in, Target: " + String.format("%.2f", Units.metersToInches(targetPosition)) + " in");
          return false;
        }
        
        System.out.println("✓ Reached mid position in " + 
            String.format("%.2f", stepTimer.get()) + " seconds");
    ...
    testSteps.add(new TestStep("Position Stability", false) {
      private Timer stepTimer = new Timer();
      ...
      @Override
      protected boolean isStepComplete() {
        return stepTimer.hasElapsed(1.0);
      }
      ...
      protected boolean validateStep() {
        ...
        if (drift > Units.inchesToMeters(1.0)) {
          setFailureReason("WARNING: Position drifted " + 
              String.format("%.2f", Units.metersToInches(drift)) + " inches while idle");
          return false;
        }
        
        System.out.println("✓ Position stable, drift: " + 
            String.format("%.3f", Units.metersToInches(drift)) + " inches");
        return true;
      }
    });
    ...
    testSteps.add(new TestStep("Return to Home", false) {
      ...
      protected void onStart() {
        stepTimer.restart();
        elevator.setWantedState(Elevator.WantedState.MOVE_TO_POSITION, 
