...
/**
 * Abstract base class for subsystem testers. Provides common functionality for running isolated
 * subsystem tests.
 */
public abstract class SubsystemTester {
  ...
  /** Define the test steps for this subsystem. Called during construction. */
  ...
  public void startTest() {
    ...
    // Reset all test steps to allow repeating tests
  ...
  public void periodic() {
    ...
    if (currentTestStep < testSteps.size()) {
      ...
      if (step.isComplete()) {
        ...
        if (!passed && step.isCritical()) {
          ...
          healthMonitor.addCheck(
              step.getName(), false, "CRITICAL FAILURE: " + step.getFailureReason());
      ...
      // All steps complete
      testState =
          healthMonitor.getOverallHealth() == SubsystemHealthMonitor.HealthStatus.FAIL
              ? TestState.FAILED
              : TestState.PASSED;
    ...
    if (currentTestStep < testSteps.size()) {
      Logger.recordOutput(
          "Testing/" + subsystemName + "/CurrentStepName",
  ...
  protected abstract class TestStep {
    ...
    /** Reset step state for repeating tests. */
    public void reset() {
      started = false;
      failureReason = "";
    }
