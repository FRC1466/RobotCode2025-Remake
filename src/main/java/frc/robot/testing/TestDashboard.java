...
/**
 * Dashboard interface for displaying test results and controlling tests. Publishes data to
 * NetworkTables for Shuffleboard/AdvantageScope visualization.
 */
public class TestDashboard {
  ...
  /** Update all dashboard outputs. Call this periodically (in Robot.robotPeriodic() or similar). */
  public void updateDashboard() {
    ...
    if (activeTester != null) {
      ...
      SmartDashboard.putString(
          "Test/Health", activeTester.getHealthMonitor().getOverallHealth().toString());

      Logger.recordOutput("Test/State", activeTester.getTestState().name());
      Logger.recordOutput("Test/Progress", activeTester.getProgress());
      Logger.recordOutput(
          "Test/HealthStatus", activeTester.getHealthMonitor().getOverallHealth().toString());
      ...
      summary.append(
          String.format("Checks: %d Pass, %d Fail, %d Warn\n", passCount, failCount, warnCount));
      summary
          .append("Overall: ")
          .append(activeTester.getHealthMonitor().getOverallHealth().toString());
